...
final class CustomerCopilotEngine {
    ...
    private func completeProgressiveAnswer(
        ...
        if let legacy {
            Task {
                await interviewAnswerSaveHandler(
                    sessionID,
    ...
    private func startReferenceGeneration(
        ...
        referenceTask = Task { [weak self] in
            ...
            do {
                ...
                await self.interviewAnswerSaveHandler(
                    self.sessionID,
                    InterviewHistoryAnswer(
                        id: request.id,
                        createdAt: Date(),
    ...
    private func archiveCurrentCueIfNeeded() {
        guard let cue = supplementalSuggestion ?? suggestion,
              cue.confidence != "low" || activeProvider != .local,
              !currentQuestion.isEmpty else { return }
        recentCues.insert(RecentInterviewCue(id: UUID(), question: currentQuestion, cue: cue), at: 0)
