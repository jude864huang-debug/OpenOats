...
final class CustomerCopilotEngineReferenceTests: XCTestCase {
    ...
    func testPromptContainsVersionedContractAndDoesNotAnchorToFastCue() async {
        ...
        XCTAssertTrue(prompt.contains("<PROGRESSIVE_ANSWER_CONTRACT version=\"progressive-answer-v2\">"))
    ...
    private func makeHarness(
        ...
        let root = FileManager.default.temporaryDirectory
            .appendingPathComponent("openoats-progressive-tests-\(UUID().uuidString)", isDirectory: true)
        try? FileManager.default.createDirectory(at: root, withIntermediateDirectories: true)
        let suiteName = "com.openoats.progressive-tests.\(UUID().uuidString)"
        ...
        return Harness(engine: engine, transcriptStore: transcriptStore)
    }
