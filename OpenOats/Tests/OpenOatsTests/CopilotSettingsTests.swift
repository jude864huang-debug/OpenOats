...
final class CopilotSettingsTests: XCTestCase {
    ...
    private func makeStore(
        ...
        let suite = defaults ?? {
            let name = "com.openoats.copilot-settings.\(UUID().uuidString)"
    ...
    func testInterviewLensInvalidStoredValuesAreNormalized() throws {
        let name = "com.openoats.copilot-lens-normalization.\(UUID().uuidString)"
    ...
    func testInterviewLensPreferencesRoundTripAcrossStoreInstances() {
        let name = "com.openoats.copilot-lens-round-trip.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: name)!
        defaults.removePersistentDomain(forName: name)
        let placements = [
    ...
    func testAutoReferenceAnswerPreferenceRoundTripsThroughDefaults() {
        let name = "com.openoats.copilot-reference-answer.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: name)!
        defaults.removePersistentDomain(forName: name)
    ...
    func testCodexSpeedPreferencesRoundTripThroughDefaults() {
        let name = "com.openoats.copilot-codex-speed.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: name)!
        defaults.removePersistentDomain(forName: name)
    ...
    func testSingleAnswerSettingsReuseLegacyModelKeysAndPersistNewControls() {
        let name = "com.openoats.copilot-single-answer.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: name)!
        defaults.removePersistentDomain(forName: name)
    ...
    func testTurnShortcutRoundTripsThroughDefaults() {
        let name = "com.openoats.copilot-hotkey.\(UUID().uuidString)"
        let defaults = UserDefaults(suiteName: name)!
        defaults.removePersistentDomain(forName: name)
    ...
    func testShortcutValidationRejectsAppConflictsAndWarnsForSystemShortcut() {
        ...
        XCTAssertEqual(
            ...
            .invalid("该组合已被 OpenOats 的现有操作使用。")
        )
